...
END AS "Monetary Score"
           ...
           , elt.trustee_flag
           , elt.spouse_of_trustee_flag
           , elt.handling_list
           , elt.opt_out_list
           --, sp_vpm.handling_list                                                                       as spouse_handling_list
           --, sp_vpm.opt_out_list                                                                        as spouse_opt_out_list
           , elt.anonymous_donor_ind
           , elt.opt_in_only_flag
           , elt.opt_in_list
           , elt."ACTIVE_WITH_RESTRICTIONS_(AWR)"
           , elt.awr_groups
     ...
     join    all_ids
	   on    (all_ids."ID Number" = elt.id_number)
     join    entity e
       on    (e.id_number = all_ids."ID Number")
left join    mr_gift_date
       on    (mr_gift_date.id_number = all_ids."ID Number")
left join    gift_info
       on    (gift_info.id_number = all_ids."ID Number")
left join    email_1
       on    (email_1."ID Number" = all_ids."ID Number")
left join    email_2
       on    (email_2."ID Number" = all_ids."ID Number")
left join    phone_1
       on    (phone_1."ID Number" = all_ids."ID Number")
left join    phone_2
       on    (phone_2."ID Number" = all_ids."ID Number")
left join    phone_3
       on    (phone_3."ID Number" = all_ids."ID Number")
LEFT OUTER JOIN PROSPECT_ENTITY PE ON all_ids."ID Number" = PE.ID_NUMBER
...
LEFT OUTER JOIN MBTI_INVITE_MASTER_LIST M ON CASE WHEN E.LAST_NAME = '' THEN elt.pref_mail_name ELSE e.last_name END = CASE WHEN M."Last Name" IS NULL THEN M."Name" ELSE M."Last Name" END AND SUBSTR(elt.zipcode,1,5) = SUBSTR(M."Zip Code",1,5) 
LEFT OUTER JOIN f_alloc f on all_ids."ID Number" = f."ID Number"
