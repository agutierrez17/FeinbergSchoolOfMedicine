CREATE OR REPLACE VIEW RPT_RVA7647.FSM_CONSECUTIVE_GIVING_YEARS AS
with cte as 
 (
...
where x = rn  -- no gap
AND 
ID_NUMBER IN (SELECT ID_NUMBER FROM CTE WHERE X = 1 AND GivingYear in (extract(year from sysdate), extract(year from sysdate) -1))
group by ID_NUMBER
)

SELECT
CTE2.ID_NUMBER,
CTE2.ConsecutiveGivingYears
FROM CTE2
